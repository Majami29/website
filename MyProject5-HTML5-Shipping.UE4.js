...
var Module = {
	...
	UE4_indexedDBVersion: 202304051806, // Bump this number to invalidate existing IDB storages in browsers.
};
...
var TASK_SHADERS = 2;
var TASK_MAIN = 3;
var loadTasks = [ 'Скачивание', 'Compiling WebAssembly', 'Building shaders', 'Launching engine'];
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://dl.dropboxusercontent.com/s/tqywqkqy6nuj8v0/UE4Game-HTML5-Shipping.wasm?dl=0'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/Majami29/website/bf8c9ed4f7211c7391e77f8e4ab768398ebb5f7e/UE4Game-HTML5-Shipping.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/Majami29/website/bf8c9ed4f7211c7391e77f8e4ab768398ebb5f7e/MyProject5-HTML5-Shipping.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/Majami29/website/bf8c9ed4f7211c7391e77f8e4ab768398ebb5f7e/Utility.js')).then(addScriptToDom);
		var dataDownload =
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://dl.dropboxusercontent.com/s/aj3ifc0ri7rfg6m/MyProject5-HTML5-Shipping.data?dl=0'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://dl.dropboxusercontent.com/s/aj3ifc0ri7rfg6m/MyProject5-HTML5-Shipping.data?dl=0')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
